...
if (Button.CENTER in ev3.buttons.pressed()):
    ...
    drive_utils.drive_till_white(robot, left_sensor)
    # Tweaking to get to exact position HCK1 Added 10 --> 20
    robot.straight(20)
    robot.stop()
    ...
    # HCK 2: Lowered distance to not pull too much // 180 --> 160 -->170 --> 175
    robot.straight(-175)

    # Turn left to pull into small circle
    # front_motor_2.run_angle(config.ARM_MOTOR_SPEED, 30, then=Stop.HOLD, wait=False)
    robot.turn(-60)
    robot.turn(60)
...
if (Button.CENTER in ev3.buttons.pressed()):
    ...
    # HCK turn increased based on run - from -75 --> -85
    robot.turn(-85)
    robot.straight(-320)
    i=0
    for i in range(0,20):
